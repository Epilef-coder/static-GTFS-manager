...
var layerControl = L.control.layers(baseLayers, null, { collapsed: true, autoZIndex: false }).addTo(map);
...
function diagnosticDataActions(returndata) {
	...
	if (!allStopsMappedFlag) {
		// Only allow to go to step 2.
		$("#Stations-tab").removeClass('disabled');
	}
	else {
		// Remove all disabled
...
function routesInfoPopulate() {
	...
	for (i in weekdaySchedules) {
		n = parseInt(i) + 1;
		weekdaySelect = `<select name="weekdaySelect${n}" id="weekdaySelect${n}" class="form-control">`;
		...
		sundaySelect = `<select name="sundaySelect${n}" id="sundaySelect${n}" class="form-control">`;
		...
		routesInfoHTML += `<p><small><label title="For internal system use. Use a unique alphanumeric id like R01">route_id: <input required id="route_id${n}" name="route_id${n}" size=6 value="R${n}" class="form-control"> <small><i>Keep it unique!</i></small></label><br>`;
		routesInfoHTML += `<label title="What commuters will see as 'bus number' types. Like '32A', 'BLUE' etc.">route_short_name: <input required id="route_short_name${n}" name="route_short_name${n}" size=6 value="R${n}" class="form-control"></label><br>`;
		routesInfoHTML += `<label title="Like: Aluva to Maharaja Collage">route_long_name: <input required id="route_long_name${n}" name="route_long_name${n}" size=6 value="Route ${n}" class="form-control"></label></small></p>`;
		routesInfoHTML += '<hr>';
	}
	routesInfoHTML += '</form>';
...
function checkMisc() {
	...
	$("#routes-check").html(checkRoutesFlag ? '<font color="green" size="6"><i class="fas fa-check"></i></font>' : '<font color="red" size="6"><i class="fas fa-times"></i></font>');
	...
	$("#calendar-check").html(checkCalendarFlag ? '<font color="green" size="6"><i class="fas fa-check"></i></font>' : '<font color="red" size="6"><i class="fas fa-times"></i></font>');
	...
	$("#agency-check").html(checkAgencyFlag ? '<font color="green" size="6"><i class="fas fa-check"></i></font>' : '<font color="red" size="6"><i class="fas fa-times"></i></font>');
	...
	$("#fares-check").html(checkFaresFlag ? '<font color="green" size="6"><i class="fas fa-check"></i></font>' : '<font color="red" size="6"><i class="fas fa-times"></i></font>');
